...
async function testRealWorkflowGeneration(prompt: string): Promise<boolean> {
  ...
  console.log(`\n🧪 Testing Real Workflow Generation`);
  console.log(`📝 Prompt: "${prompt}"`);
  console.log(`🔗 Correlation ID: ${correlationId}`);

  try {
    // Step 1: Get intent from backend
    console.log(`\n📡 Step 1: Sending to /ai/intent...`);
    ...
    if (intent.workflowPrompt) {
      ...
      console.log(`✅ Workflow generated by Groq:`);
      ...
      if (workflow.edges.length > 0) {
        ...
        workflow.edges.forEach((edge, idx) => {
          ...
          console.log(
            `   ${idx + 1}. ${sourceNode?.type || edge.source} → ${targetNode?.type || edge.target}`
          );
        });
      }
      ...
      if (invalidNodes.length > 0) {
        console.warn(`⚠️  Warning: Found ${invalidNodes.length} nodes with unknown types`);
      ...
      console.log(`ℹ️  No workflow generation needed for this intent`);
      return true;
    }
  } catch (error) {
...
async function testMultiplePrompts(): Promise<void> {
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log(`🚀 REAL INTEGRATION TEST - TAMBO + GROQ`);
  ...
  if (failed === 0) {
    ...
    console.log(`\n✨ Successfully tested:`);
    console.log(`   • Real Tambo integration`);
    console.log(`   • Real Groq workflow generation`);
    console.log(`   • Intent detection → Workflow generation flow`);
    console.log(`   • Multiple workflow types`);
    console.log(`   • Node and edge validation`);
    console.log(`\n🚀 System is production-ready with real AI!`);
    console.log(`${"=".repeat(70)}\n`);
    process.exit(0);
  } else {
    console.log(`\n❌ Fix the failing tests.`);
    console.log(`\n💡 Check:`);
    console.log(`   • GROQ_API_KEY is set in backend/.env`);
    console.log(`   • MongoDB is running`);
    console.log(`   • Backend is running (bun run dev)`);
    console.log(`${"=".repeat(70)}\n`);
    process.exit(1);
  }
}
...
async function checkPrerequisites(): Promise<boolean> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`❌ Intent endpoint returned ${response.status}`);
      return false;
    }
    console.log(`✅ Intent endpoint is working`);
  } catch (error) {
    console.error(`❌ Intent endpoint is not reachable`);
    return false;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`❌ Workflow generation endpoint returned ${response.status}`);
      console.error(`   Make sure GROQ_API_KEY is set in backend/.env`);
      return false;
    }
    console.log(`✅ Workflow generation endpoint is working`);
    console.log(`✅ Groq API is accessible`);
  } catch (error) {
    console.error(`❌ Workflow generation endpoint failed`);
...
(async () => {
  if (!(await checkPrerequisites())) {
    console.error(`\n❌ Prerequisites check failed. Fix the issues above.\n`);
    process.exit(1);
  }

  console.log(`\n✅ All prerequisites met. Starting real integration tests...\n`);
